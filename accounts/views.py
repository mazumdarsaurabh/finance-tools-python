...
def emi_calculator(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            result = calculate_emi(**form.cleaned_data)
    else:
        form = EMIForm()
    return render(request, 'accounts/emi.html', {'form': form, 'result': result})
...
def loan_eligibility_calculator(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            result = estimate_home_loan_eligibility(**form.cleaned_data)
    else:
